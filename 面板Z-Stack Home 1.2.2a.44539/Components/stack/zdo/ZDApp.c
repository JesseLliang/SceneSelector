...
UINT16 ZDApp_event_loop( uint8 task_id, UINT16 events )
{
  ...
  {
		//add by jc,for 加网失败10次后，等待1分钟再加网
		static int8 initFailCounter = 0;
		
		if(initFailCounter++ >= 10)
		{
			osal_start_timerEx(ZDAppTaskID, ZDO_NETWORK_INIT, 6000);
			initFailCounter = 0;
      ...
      else if(devStartMode == MODE_REJOIN)
      {
        if ( ZSTACK_END_DEVICE_BUILD )
        {
          ...
          else if ( prevDevState == DEV_NWK_SEC_REJOIN_ALL_CHANNEL)
          {
            ...
            //将这两句注释后，可能导致重入网到网关没保存在的节点？2018.1.18
						//不注释会使节点重加网失败，不停发Beacon request，rejoin到协调器，它也不给转发Recall Scene 2018.1.18
						//_NIB.nwkKeyLoaded = FALSE;             //add by jc，不清除旧的keys，不以TC加入的方式加网
